...
assert(runtime_design.include?("经 Controller 授权的调用方") && runtime_design.include?("POST /api/v1/projects/{id}/tasks/{taskId}/cancel") && runtime_design.include?("不得自动验收"), "runtime design must define authorized Controller cancellation without automatic acceptance")
assert(runtime_design.include?("tasks[].cancellation") && runtime_design.include?("ProjectMeta-first") && runtime_design.include?("cancellation` envelope"), "runtime design must define durable Controller cancellation retry fencing")
assert(runtime_design.include?("plan_dag` / `plan_loop`") && runtime_design.include?("不能原地改回非终态") && runtime_design.include?("不能从计划删除"), "runtime design must preserve cancellation decisions across replans")
assert(runtime_design.include?("TaskMeta.status == submitted") && runtime_design.include?("continuation.status == pending") && runtime_design.include?('submission_id != ""') && runtime_design.include?('delivery_id != ""'), "runtime design must define fail-closed PR2 continuation eligibility")
